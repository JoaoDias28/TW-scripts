    ...
    $.each(playerData, function (playerName) {
        ...
        for (const villageId of villageIds) {
            ...
            if (thisVillageAxeUnits >= minAxeAntiBunk && thisVillageLightUnits >= minLightAntiBunk && thisVillageRamUnits >= minRamAntiBunk) {
                allAntiBunk.push({ coord: village.coords, axe: thisVillageAxeUnits, lc: thisVillageLightUnits, ram: thisVillageRamUnits });
                ...
                allFullAtk.push({ coord: village.coords, axe: thisVillageAxeUnits, lc: thisVillageLightUnits, ram: thisVillageRamUnits });
            }
        }

        const villagesToDisplayAB = isFilterActive ? allAntiBunk.filter(v => (calculateDistance(v.coord, targetCoords) * unitSpeed * 60) <= maxTimeSeconds) : allAntiBunk;
        const villagesToDisplayFA = isFilterActive ? allFullAtk.filter(v => (calculateDistance(v.coord, targetCoords) * unitSpeed * 60) <= maxTimeSeconds) : allFullAtk;
        
        const countAB = villagesToDisplayAB.length;
        const countFA = villagesToDisplayFA.length;
        grandTotalAntiBunk += countAB;
        grandTotalFullAtk += countFA;
        ...
        <div class="atc-player-card" id="player${playerName}">
            ...
            <div class="atc-player-body">
                <div class="atc-category">
                    <h4>Anti-Bunk Nukes <span class="count">${countAB}</span></h4>
                    <button class="collapsible">Show Villages</button>
                    <div class="content">${abTable}</div>
                </div>
                <div class="atc-category">
                    <h4>Standard Full-Atk <span class="count">${countFA}</span></h4>
                    <button class="collapsible">Show Villages</button>
