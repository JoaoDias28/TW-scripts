                    ...
                    $.each(rows, function (rowNr) {
                        ...
                        $.each(game_data.units, function (index) {
                            ...
                            if (rows.eq(rowNr).children().not(':first').eq(index).text().trim() != '?') {
                                villageData[thisID][unitName] = rows.eq(rowNr).children().not(':first').not(':first').eq(index).text().trim();
                                villageData["total"][unitName] += parseInt(rows.eq(rowNr).children().not(':first').not(':first').eq(index).text().trim());
                            }
                            else {
